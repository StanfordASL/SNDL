...
load('learned_functions/PVTOL_raw_250_new.mat'); 
max_itr = itr;
MAX_ITR(2) = max_itr;
...
load('learned_functions/PVTOL_raw_500_new.mat');
max_itr = itr;
MAX_ITR(3) = max_itr;
CON_VIOL(:,1:max_itr,3) = constraint_viol(:,1:max_itr);
reg_err_all = zeros(max_itr,1);
viol_all = zeros(max_itr,1);
for itr = 1:max_itr
    % Define functions
...
REG_VIOL{3} = [reg_err_all,viol_all];

clearvars -except CON_VIOL MAX_ITR REG_VIOL

%%
load('learned_functions/PVTOL_raw_1000_new.mat'); 
max_itr = itr;
MAX_ITR(4) = max_itr;
CON_VIOL(:,1:max_itr,4) = constraint_viol(:,1:max_itr);
...
REG_VIOL{4} = [reg_err_all,viol_all];
...
nom_legend = {'N = 100','N = 250','N = 500','N = 1000'};
for idx = 1:4
