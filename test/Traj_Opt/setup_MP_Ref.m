...
x_eq_all = kron(interp1(tau_full', pos_ref(:,1),t_grid'),[1;zeros(5,1)])+...
           kron(interp1(tau_full', pos_ref(:,2),t_grid'),[0;1;zeros(4,1)])+...
           kron(zeros(N+1,1),zeros(6,1));
...
con = @(xu,Prob) MP_con(xu,Prob,n,m,N,P,D,f,B,Tp,x_T);
conJ = @(xu,Prob) MP_conJ(xu,Prob,n,m,N,P,D,df,dB,Tp,x_T);
